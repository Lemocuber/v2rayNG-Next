...
- The navigation drawer entry remains tappable even when requirements are unmet so the app can surface the `Shizuku permission and VPN mode are required for Proxied Only Apps` toast instead of silently swallowing taps on a disabled menu item.
